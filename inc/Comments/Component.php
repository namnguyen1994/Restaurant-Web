...
namespace Geritcht\Geritcht\Comments;
...
class Component implements Component_Interface, Templating_Component_Interface
{
	...
	public function the_comments(array $args = array())
	{
		...
		$amp_live_list = geritcht()->using_amp_live_list_comments();
...
?>
			...
			?>
				<div>
					<button class="button" on="tap:amp-live-comments-list-<?php the_ID(); ?>.update"><?php esc_html_e('New comment(s)', 'geritcht'); ?></button>
				</div>
			</amp-live-list>
			<?php
			}
	...
	if (!class_exists('WPSE_Walker_Comment')) {
		...
		class Component_Walker_Comment extends \Walker_Comment
		{
			...
			?>
					<li <?php comment_class('geritcht-comments-item'); ?> id="comment-<?php comment_ID(); ?>">
						<h4 class="mt-0 mb-0"><span class="geritcht-type-date"><span class="mr-2"><?php echo esc_html__(comment_type() . ':', 'geritcht'); ?></span><?php comment_author_link(); ?></span> <?php edit_comment_link(esc_html__('(Edit)', 'geritcht'), '<span class="edit-link">', '</span>'); ?>
						</h4>
						<div class="geritcht-comment-type-date">
				...
				?>
					<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
						<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
							<div class="geritcht-comments-media">
								<div class="geritcht-comment-wrap">
									<div class="geritcht-comments-photo">
										<?php if (0 != $args['avatar_size']) echo get_avatar($comment, $args['avatar_size']); ?>
									</div>
									<div class="geritcht-comments-info">
										<div class="geritcht-comment-metadata">
											<a href="<?php echo esc_url(get_comment_link($comment->comment_ID)); ?>">
												<time datetime="<?php comment_time('c'); ?>">
													<?php printf(wp_kses('%1$s', '1: date'), get_comment_date()); ?>
												</time>
											</a>
											<?php edit_comment_link(esc_html__('Edit', 'geritcht'), '<span class="edit-link">', '</span>'); ?>
										</div>
										<!-- .comment-metadata -->
										<h4 class="title">
											<?php printf(wp_kses('%s ', 'geritcht'), sprintf('%s', get_comment_author_link())); ?>
										</h4>

										<?php if ('0' == $comment->comment_approved) : ?>
											<p class="comment-awaiting-moderation"><?php esc_html__('Your comment is awaiting moderation.', 'geritcht'); ?></p>
										<?php endif; ?>
										<div class="comment-content">
											<?php comment_text(); ?>
										</div><!-- .comment-content -->
										<?php geritcht()->geritcht_get_comment_reply($depth); ?>
										<!-- .reply -->
									</div><!-- .comment-author -->
