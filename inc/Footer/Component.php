...
namespace Geritcht\Geritcht\Footer;
...
class Component implements Component_Interface, Templating_Component_Interface
{
	...
	public function register_footers($footer_option)
	{
		...
		$theme_option = get_option('geritcht-options');

		$default = [
			'1' => esc_html__('text-left', 'geritcht'),
			'2' => esc_html__('text-right', 'geritcht'),
			'3' => esc_html__('text-center', 'geritcht'),
		];
		...
		foreach ($footer_option as $key => $item) {
			...
			register_sidebar(
				array(
					'name'          => esc_html__('Footer Area ' . $key, 'geritcht'),
					'class'         => 'nav-list',
	...
	public function get_footer_option(): array
	{
		...
		if (
			...
			if (class_exists('ReduxFramework')) {
				...
				$options = !empty($geritcht_options['geritcht_footer_width']) ? $geritcht_options['geritcht_footer_width'] : '';
