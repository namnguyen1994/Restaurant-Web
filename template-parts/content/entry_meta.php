...
<div class="geritcht-blog-meta">
	<ul class="list-inline">
		<?php
		if (!empty($author_string)) { ?>
			<li class="posted-by">
				<?php
				_e('Posted by : ','geritcht');
				/* translators: %s: post author */
				$author_byline = _x('By %s', 'post author', 'geritcht');
				if (!empty($time_string)) {
					/* translators: %s: post author */
					$author_byline = _x('%s', 'post author', 'geritcht');
				}
				printf(
		...
		if (!empty($parent_string)) { ?>
			<li class="posted-in">
				...
				$parent_note = _x('In %s', 'post parent', 'geritcht');
